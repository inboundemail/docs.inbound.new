...
<RequestExample>
```bash cURL
curl -X GET 'https://api.inbound.email/api/v2/domains/dom_abc123?check=true' \
  -H 'Authorization: Bearer YOUR_API_KEY'
```

```javascript JavaScript
const response = await fetch('https://api.inbound.email/api/v2/domains/dom_abc123?check=true', {
  headers: {
    'Authorization': 'Bearer YOUR_API_KEY'
  }
});
