...
<ResponseField name="mailFromDomain" type="string | null">
  Configured MAIL FROM domain (if set). This eliminates "via amazonses.com" attribution.
</ResponseField>

<ResponseField name="mailFromDomainStatus" type="string | null">
  MAIL FROM domain status: `Pending`, `Success`, `Failed`, or `NotSet`.
</ResponseField>

<ResponseField name="mailFromDomainVerifiedAt" type="string | null">
...
<ResponseField name="verificationCheck" type="object" optional>
  ...
  <Expandable title="Verification check properties">
    ...
    <ResponseField name="dkimStatus" type="string" optional>
      DKIM verification status: `Pending`, `Success`, `Failed`, or `NotFound`.
    </ResponseField>
    <ResponseField name="dkimVerified" type="boolean" optional>
      Whether DKIM is fully verified and enabled.
    </ResponseField>
    <ResponseField name="dkimTokens" type="array" optional>
      Array of DKIM tokens configured for the domain.
    </ResponseField>
    <ResponseField name="mailFromDomain" type="string" optional>
      Configured MAIL FROM domain (if set).
    </ResponseField>
    <ResponseField name="mailFromStatus" type="string" optional>
      MAIL FROM domain status: `Pending`, `Success`, `Failed`, or `NotSet`.
    </ResponseField>
    <ResponseField name="mailFromVerified" type="boolean" optional>
...
<ResponseField name="authRecommendations" type="object" optional>
  Recommended DNS records to improve deliverability (present when SPF/DMARC are missing).
  <Expandable title="Recommendations">
    <ResponseField name="spf" type="object" optional>
      Suggested SPF record for the root domain.
      
      <Expandable title="SPF record properties">
    ...
    <ResponseField name="dmarc" type="object" optional>
      Suggested DMARC record with p=none for monitoring.
      
      <Expandable title="DMARC record properties">
        <ResponseField name="name" type="string">
...
curl_setopt_array($curl, array(
  CURLOPT_URL => 'https://inbound.new/api/v2/domains/dom_abc123',
...
curl_setopt_array($curl, array(
  CURLOPT_URL => 'https://inbound.new/api/v2/domains/dom_abc123?check=true',
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_HTTPHEADER => array(
    'Authorization: Bearer YOUR_API_KEY'
  ),
