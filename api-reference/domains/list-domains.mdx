...
const { data: domains, error } = await inbound.domain.list()
if (error) {
...
<?php
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://inbound.new/api/v2/domains');
...
if (error) {
  ...
  console.log('Verified domains:', domains.data)
}
```
...
<?php
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://inbound.new/api/v2/domains?status=verified&check=true&limit=10');
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_HTTPHEADER, [
    'Authorization: Bearer YOUR_API_KEY'
]);
