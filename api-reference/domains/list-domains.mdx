...
This endpoint retrieves a paginated list of all domains associated with the authenticated user. It supports filtering by domain status and whether domains can receive emails.
...
<RequestExample>
```bash cURL
curl -X GET 'https://api.inbound.email/api/v2/domains?status=verified&limit=10' \
  -H 'Authorization: Bearer YOUR_API_KEY'
```

```javascript JavaScript
const response = await fetch('https://api.inbound.email/api/v2/domains?status=verified&limit=10', {
  headers: {
    'Authorization': 'Bearer YOUR_API_KEY'
  }
});
