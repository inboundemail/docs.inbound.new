...
const { data: emailAddress, error } = await inbound.email.address.get('email_addr_123')

if (error) {
  console.error('Error:', error)
} else {
  console.log('Email address:', emailAddress.address)
  console.log('Domain:', emailAddress.domain.name)
  console.log('Routing type:', emailAddress.routing.type)
}
```
...
<?php
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://inbound.new/api/v2/email-addresses/email_addr_123');
