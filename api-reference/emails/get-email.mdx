...
<RequestExample>
  ```bash cURL
  curl -X GET 'https://api.inbound.email/api/v2/emails/em_1234567890abcdef' \
       -H 'Authorization: Bearer your_api_key' \
       -H 'Content-Type: application/json'
  ```

  ```javascript Node.js
  const response = await fetch('https://api.inbound.email/api/v2/emails/em_1234567890abcdef', {
    headers: {
  ...
  response = requests.get(
    'https://api.inbound.email/api/v2/emails/em_1234567890abcdef',
    headers={
      'Authorization': 'Bearer your_api_key',
      'Content-Type': 'application/json',
    }
  ...
  ```php PHP
  $ch = curl_init('https://api.inbound.email/api/v2/emails/em_1234567890abcdef');
