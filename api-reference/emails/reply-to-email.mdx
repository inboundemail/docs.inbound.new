...
api: "POST https://api.inbound.email/api/v2/emails/{id}/reply"
...
<CodeGroup>
```bash API Key
curl -X POST https://api.inbound.email/api/v2/emails/{id}/reply \
...
<RequestExample>
```bash cURL
curl -X POST https://api.inbound.email/api/v2/emails/email_abc123/reply \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```javascript Node.js
const response = await fetch('https://api.inbound.email/api/v2/emails/email_abc123/reply', {
  method: 'POST',
  headers: {
...
response = requests.post(
    'https://api.inbound.email/api/v2/emails/email_abc123/reply',
    headers={
        'Authorization': 'Bearer YOUR_API_KEY',
        'Content-Type': 'application/json'
    },
