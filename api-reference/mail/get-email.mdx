...
// Get email details
const { data: email, error } = await inbound.mail.get('email_123')
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://inbound.new/api/v2/mail/{$email_id}");
...
const getEmailDetails = async (emailId) => {
  const { data: email, error } = await inbound.mail.get(emailId)
