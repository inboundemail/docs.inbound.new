...
```bash cURL
curl -X PATCH "https://api.inbound.email/v2/mail/email_123" \
...
```javascript Node.js
const response = await fetch('https://api.inbound.email/v2/mail/email_123', {
  method: 'PATCH',
  headers: {
...
response = requests.patch(
    'https://api.inbound.email/v2/mail/email_123',
    headers={
        'Authorization': 'Bearer YOUR_API_KEY',
        'Content-Type': 'application/json'
    },
