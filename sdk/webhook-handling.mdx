...
description: "Handle incoming email webhooks with type safety using SDK v3.0.0"
---

## Overview

When emails arrive at your configured addresses, Inbound sends a webhook to your endpoint with the complete email data. SDK v3.0.0 provides improved TypeScript types and streamlined reply handling.
...
<CodeGroup>
```typescript app/api/webhook/route.ts (v3.0.0)
...
```typescript app/api/webhook/route.ts (with database - v3.0.0)
import { NextRequest, NextResponse } from 'next/server'
import { Inbound, isInboundWebhook } from '@inboundemail/sdk'
      ...
      attachments: [
        {
          ...
          downloadUrl: 'https://inbound.new/api/v2/attachments/test_email_123/document.pdf'
        }
      ],
      headers: {},
      priority: undefined
...
Attachments in webhook payloads include a `downloadUrl` field for easy file access:
...
export async function POST(request: NextRequest) {
  ...
  for (const attachment of email.parsedData.attachments) {
    ...
    // Download the attachment if needed
    ...
    if (fileResponse.ok) {
      ...
      // Save to cloud storage, process, etc.
      await saveToS3(attachment.filename, fileData)
    }
  }
...
<Tip>
The `downloadUrl` is authenticated via your API key, ensuring only you can access attachments from your emails. The URL includes the filename to enable proper content-type handling.
</Tip>
